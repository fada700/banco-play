...
async function ensureUsuario(discord: {
  ...
  if (existing?.auth_user_id) {
    // Actualizar nombre/avatar por si cambiaron
    await supabaseAdmin
      .from("usuarios")
  ...
  // Insertar usuario (trigger crea tarjeta debito + rol usuario + tarjeta credito sin_solicitar)
...
async function syncRoles(usuarioId: string, discordUserId: string): Promise<void> {
  ...
  // borrar admin/trabajador previos para reflejar el estado actual
  await supabaseAdmin
    .from("roles_usuario")
    .delete()
...
// =============================================================
// PASO 1: callback OAuth — intercambia code, crea/actualiza
// usuario, genera código 4 dígitos, lo envía por DM y devuelve
// un session_id temporal que el cliente usa para verificar.
// =============================================================
export const startLogin = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    try {
      ...
      throw new Error(
        "No se pudo enviar el código por DM. Asegúrate de tener DMs abiertos en el servidor.",
      );
    }
...
// =============================================================
// PASO 2: verificar código 4 dígitos. Si OK, devuelve email/password
// para que el cliente abra sesión en Supabase.
// =============================================================
export const verifyCode = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (hash !== row.codigo_hash) {
      ...
      // incrementar intentos en usuario; si llega a 3, bloquear 5 min
